...
- Completely refactored URL replacement logic from `tabs.onCreated` to `windows.onCreated` and `runtime.onStartup`:
  - The extension now correctly only replaces the startup URL when a completely **new window** is opened or when the browser first launches.
  - Implemented a robust retry mechanism to ensure the initial tab of the first window is correctly captured and redirected, even on slower startups.
  - Ignores all newly opened tabs within an existing window.
